...
app.MapGet(
        ...
        async (string key, ICacheService cache) =>
        {
            return await cache.GetOrCreateAsync(
                key,
                () => Task.FromResult($"{nameof(cache.GetOrCreateAsync)} - Hello World")
            );
        }
    )
...
app.MapGet(
        ...
        async (string key, ICacheService cache) =>
        {
            return await cache.GetOrDefaultAsync(
                key,
                $"{nameof(cache.GetOrDefault)} - Hello World"
            );
        }
    )
...
app.MapDelete(
        "/RemoveAsync",
        (string key, ICacheService cache) =>
        {
